...
//server.c  
//Project 3
//This server.c will take in the information passed in from the client.c and perform the actions and then write back to the client.
#include "csapp.h"
...
void serverFunction(int connfd,char client_hostname[MAXLINE],char client_port[MAXLINE]){
    ...
    if(action_option == 1){            
        ...
        bzero(buffer,MAXLINE);
        fclose(file_object);  //Closed the file 
    }
